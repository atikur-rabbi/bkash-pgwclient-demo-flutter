...
files on your hosting & change the `config.json` with <mark> sandbox <mark> credential <span style="color:blue">(Not work with live credential)</span> `app_key`, `app_secret`, `username` & `password`. Nothing else needs to change.
