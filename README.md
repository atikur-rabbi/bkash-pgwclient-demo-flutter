...
files on your hosting & change the `config.json` with ```diff -sandbox ``` credential <span style="color:blue">(Not work with live credential)</span> `app_key`, `app_secret`, `username` & `password`. Nothing else needs to change.
